...
def rotate_point_around_point(x, y, cen_x, cen_y, theta):

    cosTheta = math.cos(theta)
    sinTheta = math.sin(theta)
    rot_x = (cosTheta * (x - cen_x) - sinTheta * (y - cen_y) + cen_x)
    rot_y = (sinTheta * (x - cen_x) + cosTheta * (y - cen_y) + cen_y)

    return rot_x, rot_y


def arrow_line(image, x1, y1, x2, y2, arrow_size, color, thickness, lineType):
    distance = ((x2-x1)**2 + (y2-y1)**2)**0.5

    t = 1.0 - (arrow_size*thickness)/distance
    x3 = x2 * t + x1 * (1 - t)
    y3 = y2 * t + y1 * (1 - t)
    ...
    rot_x1, rot_y1 = rotate_point_around_point(x3, y3, x2, y2, angle)
    rot_x2, rot_y2 = rotate_point_around_point(x3, y3, x2, y2, -angle)

    pts = np.array([(int(x2), int(y2)), (int(rot_x1), int(rot_y1)), (int(x4), int(y4)), (int(rot_x2), int(rot_y2))],
                   np.int32)
    cv2.fillPoly(image, [pts], color=color)
    cv2.line(image, (int(x1), int(y1)), (int(x4), int(y4)), color, thickness, lineType)

def get_abc_factor_of_line(x1, y1, x2, y2):
    dx = x2 - x1
    dy = y2 - y1
...
def is_same_size(x1, y1, x2, y2, x3, y3, x4, y4):
    a, b, c = get_abc_factor_of_line(x1, y1, x2, y2)
    f1 = a * x3 + b * y3 + c
    f2 = a * x4 + b * y4 + c
    same_size = (f1*f2 >= 0)
    return same_size
...
def get_a_line_perpendicular_to_another_line(x1, y1, x2, y2, side_point, length):
    line_vector = (x2-x1, y2-y1)
    magnitude = ((x2 - x1) ** 2 + (y2 - y1) ** 2) ** 0.5

    normalization_vector = (line_vector[0]/magnitude, line_vector[1]/magnitude)

    vector_1 = (normalization_vector[1], - normalization_vector[0])
    vector_2 = (-normalization_vector[1], normalization_vector[0])

    mid_x, mid_y = (x1 + x2)/2, (y1 + y2)/2

    point_1 = (mid_x + vector_1[0]*length, mid_y + vector_1[1]*length)
    point_2 = (mid_x + vector_2[0] * length, mid_y + vector_2[1] * length)

    if is_same_size(x1, y1, x2, y2, side_point[0], side_point[1], point_1[0], point_1[1]):
        return (mid_x, mid_y), point_1
    else:
        return (mid_x, mid_y), point_2