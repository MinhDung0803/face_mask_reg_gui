...
# sys.path.append("/media/gg-ai-team/DATA/TaiNH/FaceMaskDetection")
	...
	while True:
		...
		if event_queue.empty() != True:
			...
			if (command == "stop"):
				...
				pausing = not pausing
				if (pausing): print("Detecting and Recognizing Threading is pausing")
		...
		if is_closed_all(list_closed_queue):
			...
			print('Detection waiting for stop')
...
def detecting_test(list_frame_image_buffer, detect_step_list, video_infor_list, detection_list_list_region,
				   list_detected_buffer, no_job_sleep_time, event_queue, wait_stop):
...
def detecting_by_threading_test(list_frame_image_buffer, detect_step_list, video_infor_list, detection_list_list_region,
								list_detected_buffer, no_job_sleep_time, event_queue, wait_stop):
	t = threading.Thread(target=detecting_test,
						 args=[list_frame_image_buffer, detect_step_list, video_infor_list, detection_list_list_region,
							   list_detected_buffer, no_job_sleep_time, event_queue, wait_stop])
	t.start()